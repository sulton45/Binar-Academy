...
import com.example.ProjectBinarFood.services.EmailService;
...
public class UsersController {
    ...
    public ResponseEntity<String> addUser(@RequestBody UsersDTO usersDTO) {
        try {
            // Tambahkan pengguna ke database dengan status verifikasi email false
            Users user = usersService.addUser(usersDTO.getUsername(), usersDTO.getEmailAddress(), usersDTO.getPassword());
            if (user != null) {
                // Lakukan pengiriman email verifikasi di sini
                String verificationToken = usersService.generateVerificationToken(); // Buat token verifikasi
                usersService.sendVerificationEmail(user.getEmailAddress(), verificationToken); // Kirim email verifikasi

                // Kirim respons bahwa pengguna berhasil dibuat
                return ResponseEntity.ok("User Berhasil Dibuat. Cek email untuk verifikasi.");
            } else {
